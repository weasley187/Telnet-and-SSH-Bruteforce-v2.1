...
# as you requested it, so here you go. Keep in mind it can crash your VPS if it overheats.
...
class scan:
	def __init__(self, ip, port, user, passw):
		...
		self.passw = passw
	def is_valid(self):
		if self.ip > 1 and len(self.ip) >= 7 and self.port == "23" and timeout_int <= 61 and timeout_int >= 2: # <--- remove or comment this
